...
STAGED_MODEL_DIRNAME = Path(__file__).resolve().parents[1] / "text_recognizer" / "artifacts"
MODEL_FILE = "model.pt"
...
class ParagraphTextRecognizer:
    ...
    def __init__(self, model_path=None):
        ...
        self.ignore_tokens = [i for i, token in enumerate(self.mapping) if token in ("<S>", "<E>", "<P>")]
        ##self.ignore_tokens = [  self.mapping.index('<S>'),
        ##                        self.mapping.index('<E>'),
        ##                        self.mapping.index('<P>')
        ##                     ]
