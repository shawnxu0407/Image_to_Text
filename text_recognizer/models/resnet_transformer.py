...
class ResnetTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_pos_encoder = PositionalEncodingImage(
            d_model=self.dim, max_h=self.input_dims[1], max_w=self.input_dims[2], persistent=True
        )  # Max (Ho, Wo)
        ...
        self.dec_pos_encoder = PositionalEncoding(d_model=self.dim, max_len=self.max_output_length, persistent=True)
